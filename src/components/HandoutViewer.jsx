...
const BlankWord = ({ text, globalShow }) => {
  ...
  if (text === '' || text === '　' || text === '✓' || text === '☑️') {
     ...
     return (
       <span 
         onClick={(e) => { if (checkTool()) return; e.stopPropagation(); setLocalShow(!localShow); }} 
         className="cursor-pointer inline-block text-center w-6 data-blankword-check select-none" 
         data-text={isCheck ? '✓' : '　'}
       >
...
const parseTask1 = (text, globalShow) => {
  ...
  return parts.map((part, i) => {
    if (i % 2 === 1) {
      return <BlankWord key={i} text={part} globalShow={globalShow} />;
    }
...
const QuizBlock = ({ qIdx, q, options, a, showAllAnswers }) => {
  ...
  return (
    <div className="mb-6 cursor-pointer select-none group" onClick={(e) => { if (checkTool()) return; setShowLocal(!showLocal); }} title="點擊顯示/隱藏解答">
...
const Task4QA = ({ q, a, showAllAnswers }) => {
  ...
  const isShow = showAllAnswers || showLocal;
  return (
    <div className="cursor-pointer select-none group" onClick={(e) => { if (checkTool()) return; setShowLocal(!showLocal); }} title="點擊顯示/隱藏解答">
  ...
  const exportToWord = (mode) => {
    ...
    if (mode === 'student') {
        clone.querySelectorAll('.data-blankword').forEach(el => {
            ...
            el.style.color = '#000'; el.style.border = 'none'; el.style.background = 'transparent'; el.style.textDecoration = 'none';
        });
    ...
    <head>
      <meta charset="utf-8">
      <style>
        @page WordSection1 { size: ${sizeCss}; margin: ${marginCss}; mso-header-margin: 35.4pt; mso-footer-margin: 35.4pt; mso-paper-source: 0; }
    ...
    link.href = url; link.download = `講義_${lesson.lessonNum}_${mode === 'teacher' ? '教用' : '學用'}.doc`;
          ...
          <section className="mb-10">
            ...
            <div className="mt-6">
              ...
              <div className="mb-6">
                ...
                <div className="text-slate-700 leading-relaxed" style={{ marginLeft: '4em' }} dangerouslySetInnerHTML={{__html: lesson.task4.analyst}}></div>
              </div>
              {lesson.task4.magic && (
              <div className="mb-6">
                <div className="font-bold text-slate-800 mb-2" style={{ marginLeft: '2em' }}>◎主題魔法書</div>
                <div className="text-slate-700 leading-relaxed" style={{ marginLeft: '4em' }} dangerouslySetInnerHTML={{__html: lesson.task4.magic}}></div>
              </div>
              )}
